...
import flagsData from "./countyFlags.json";
import { Link } from "react-router-dom";

const MatchCountyFlags = () => {
  ...
  const handleAnswer = (selectedCountry) => {
    const correctCountry = flags[questionIndex].county;
  ...
  const renderOptions = () => {
    const options = [];
    const countries = [flags[questionIndex].county];
    while (countries.length < 4) {
      const randomIndex = Math.floor(Math.random() * flags.length);
      const randomCountry = flags[randomIndex].county;
  ...
  const handleWhatsappShareButton = () => {
    const game = "http://localhost:5173/match-county-flags";
    const message = `
        ...
        <Box>
          ...
          {showHint && (
            <Text fontSize="lg" mb={4}>
              ...
              {flags[questionIndex].county.charAt(0)}
            </Text>
          )}
...
export default MatchCountyFlags;
